...
public class EffectRosterViewController extends LmlViewController {
    ...
    public void onViewCreated(Group sceneRoot) {
        ...
        // Motion Blur (MIX)
        {
            MotionBlurEffect filter = new MotionBlurEffect(Pixmap.Format.RGBA8888, MixEffect.Method.MIX, 0.75f, false);
            effectsRoster.add(new EffectEntryModel("Motion Blur (MIX)", filter));
        }
        // Motion Blur (MAX)
        {
            MotionBlurEffect filter = new MotionBlurEffect(Pixmap.Format.RGBA8888, MixEffect.Method.MAX, 0.75f, false);
